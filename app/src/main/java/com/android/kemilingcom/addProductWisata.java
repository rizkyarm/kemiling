...
public class addProductWisata extends AppCompatActivity implements LocationSelectedListener {

    private static final int PICK_IMAGE_REQUEST = 1;
    private EditText editWisata, editWeekday, editWeekend, editJam, editAlamat, editWA, editDeskripsi;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_product_wisata);
        ...
        // Initialize elements
        editWisata = findViewById(R.id.editwisata);
        editWeekday = findViewById(R.id.editweekday);
        editWeekend = findViewById(R.id.editweekend);
        ...
        btnPilihMap.setOnClickListener(v -> {
            ...
            MapsFragment mapsFragment = new MapsFragment("addProductWisata");
    ...
    private void fetchAndStoreUserId() {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    if (jsonResponse.getString("status").equals("success")) {
                        ...
                        Toast.makeText(addProductWisata.this, "User ID: " + userId, Toast.LENGTH_SHORT).show();

                        sendData(userId);
                    } else {
                        Toast.makeText(addProductWisata.this, jsonResponse.getString("message"), Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                Toast.makeText(addProductWisata.this, "Error fetching user ID", Toast.LENGTH_SHORT).show();
            }
        }) {
            @Override
    ...
    public void onLocationSelected(LatLng latLng, String contextType) {
        if (contextType.equals("addProductWisata")) {
            // Handle location for addProductWisata
    ...
    private void sendData(int userId) {
        String wisata = editWisata.getText().toString().trim();
        String weekday = editWeekday.getText().toString().trim();
        String weekend = editWeekend.getText().toString().trim();
        ...
        if (wisata.isEmpty() || weekday.isEmpty() || weekend.isEmpty() || jam.isEmpty() || alamat.isEmpty() || wa.isEmpty() || deskripsi.isEmpty()) {
            Toast.makeText(addProductWisata.this, "Harap isi semua bidang", Toast.LENGTH_SHORT).show();
            return;
        }

        String url = "https://store.kemiling.com/side-page/Pendaftaran/api_addProductWisata.php?user_id=" + userId;
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put("nama_produk", wisata);
            jsonObject.put("kategori", "Tempat Wisata");
            jsonObject.put("weekday_ticket", weekday);
            jsonObject.put("weekend_ticket", weekend);
        ...
        StringRequest request = new StringRequest(Request.Method.POST, url, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                Toast.makeText(addProductWisata.this, "Data berhasil dikirim", Toast.LENGTH_SHORT).show();
                Intent intent = new Intent(addProductWisata.this, activityMyProduct.class);
            ...
            @Override
            public void onErrorResponse(VolleyError error) {
                Toast.makeText(addProductWisata.this, "Gagal mengirim data: " + error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        }) {
            @Override
            public byte[] getBody() {
